...
class ChromeGUIAgent(BaseAgent):
 
    info = {"name": "ChromeGUI","can do": "specializes in handling tasks related to the Chrome browser using GUI operations, such as reading webpages, opening downloads, and modifying Chrome settings.","can't do": "cannot handle tasks using the command line and cannot manage tasks outside the Chrome browser."}
...
if __name__ == '__main__':
    ...
    for example_id in tasks[0:]:
        ...
        gui_agent = ChromeGUIAgent(args, example, environment, action_space, observation_type, max_trajectory_length, max_steps=10)
        gui_agent.run()
